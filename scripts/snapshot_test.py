    ...
    for proto_dir in proto_dirs:
        ...
        for proto_file in proto_files:
            cmd = build_protoc_command(
                proto_dir, proto_dir, "__gen", PROJECT_ROOT,
                proto_file.name, include_path
            )
            
            try:
                subprocess.run(cmd, check=True, capture_output=True, text=True)
            except subprocess.CalledProcessError as e:
                logger.error(f"Protoc failed for {proto_dir.name}/{proto_file.name}: {e}")
                if e.stderr:
    ...
    for proto_dir in proto_dirs:
        ...
        if gen_dir.exists() and any(gen_dir.iterdir()):
            ...
            try:
                ...
                logger.error(f"Moon check failed for generated code in: {proto_dir.name}/__gen")
...
def main():
    ...
    if has_changes:
        ...
        logger.info("To update snapshots, run: python scripts/snapshot_test.py --update-snapshots")
        sys.exit(1)
